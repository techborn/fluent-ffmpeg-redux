...
function createSizeFilters(output, key, value) {
    // Store parameters
    let data = output.sizeData = output.sizeData || {};
    data[key] = value;
    ...
    let fixedSize = data.size.match(/([0-9]+)x([0-9]+)/);
    let fixedWidth = data.size.match(/([0-9]+)x\?/);
    let fixedHeight = data.size.match(/\?x([0-9]+)/);
    let percentRatio = data.size.match(/\b([0-9]{1,3})%/);
    let width, height, aspect;

    if (percentRatio) {
        let ratio = Number(percentRatio[1]) / 100;
        return [{
            filter: 'scale',
            options: {
...
module.exports = function (proto) {
    ...
    proto.size = function (size) {
        let filters = createSizeFilters(this._currentOutput, 'size', size);
    ...
    */
    proto.aspectRatio = function (aspect) {
        ...
        let filters = createSizeFilters(this._currentOutput, 'aspect', a);
    ...
    */
    proto.autopad = function (pad, color) {
        ...
        let filters = createSizeFilters(this._currentOutput, 'pad', pad ? color || 'black' : false);
